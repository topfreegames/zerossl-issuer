...
func NewClusterIssuerReconciler(k8sClient client.Client, scheme *runtime.Scheme, recorder record.EventRecorder, maxConcurrentReconciles int) *ClusterIssuerReconciler {
...
func (r *ClusterIssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.validateClusterIssuer(ctx, clusterIssuer); err != nil {
		...
		r.recorder.Event(clusterIssuer, corev1.EventTypeWarning, "ValidationFailed", fmt.Sprintf("Failed to validate cluster issuer configuration: %v", err))
	...
	r.recorder.Event(clusterIssuer, corev1.EventTypeNormal, "ClusterIssuerReady", "ClusterIssuer configuration validated and ready for use")
